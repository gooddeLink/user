...
function CCamera(){
        ...
        if (take){
            take.onchange=function(event){
                ...
                if (files && files.length>0) {
                    ...
                    try{//오류있을수도 있는 문장
                        console.log('file=',latestFile.current);
                        ...
                        console.log('imgurl=',latestImgUrl.current);
    ...
    function sendData(){
        ...
        if(latestFile.current) console.log('yes!');
        var formData=new FormData();
        formData.append("file",latestFile.current);

        axios.post("/api/change",//해당 주소로 imgFIle:latestFile.current 정보 보냄
            //{imgFile : latestFile.current}
            formData, {
                headers: {
                    "Content-Type" : "multipart/form-data" 
                }
        }).then(function(response){
            console.log('response',response);//해당 주소로부터 넘어오는 정보
