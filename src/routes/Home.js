...
function Home(){
    ...
    useEffect(()=>{//로딩시간동안 wait a minutes 뜨도록 하기 위함
        ...
        if (navigator.geolocation && google.maps) {//구글지도가 로딩되고, geolocation 기능이 동작한다면
            ...
            navigator.geolocation.getCurrentPosition(function(position) {
                ...
                setMarker = new google.maps.Marker({position: {lat: setLat, lng: setLon}, map: latestMap.current});
                ...
                google.maps.event.addListener(latestMap.current, 'click', function(event){
                    ...
                    latestMarker.current=new google.maps.Marker({position: {lat: event.latLng.lat(), lng: event.latLng.lng()}, map: latestMap.current});
    ...
    function latlon2addr(lat, lon){
        ...
        latestGeocoder.current.geocode({'location':{lat: lat, lng: lon}},function(results,status){
            if (status==='OK') {
                makebox(results[0].formatted_address,latestMap.current);//주소표시
            }
    ...
    function makebox(addr){
        ...
        infowindow.open(latestMap.current, latestMarker.current);
    ...
    function optionSelect(event){
        var selectedValue = event.currentTarget.innerHTML;//선택한 항목 글자
        setType=event.currentTarget.value;//재난유형 번호
        latestType.current=setType;
        document.querySelector(".base").innerHTML = selectedValue;//선택한 항목 글자가 나타나게 함
    }
            ...
            if (files && files.length>0) {
                ...
                try{//오류있을수도 있는 문장
                    ...
                    if(flag==='a'){
                        ...
                        show2.src=latestImgUrl.current;//사진 주소 설정
                    ...
                    if(flag==='b'){
                        ...
                        show1.src=latestImgUrl.current;//사진 주소 설정
                        styleSetCamImg1({display:'none'});//사진 선택할 이미지로고 안보이게
                        styleSetImgShow1({display:'block'});//선택한 사진 보이게

                        axios.post(`http://127.0.0.1:5000/${id}/camimgsubmit`, {//정보 전달할 페이지-ai
                            img:latestImgUrl.current
                        })
                        .then((res)=>{//axios.post 성공하면
                            console.log(res);
                    ...
                    revoke.onload=function(){link.revokeObjectURL(imgUrl);}//이미지 띄우고 url 취소하기(메모리 절약 위함)
                }
                catch(e){//에러 있다면
                    console.log('error');
                    try{
    ...
    <>
        ...
        <div className="boxgroup" style={styleCamHead}> {/*재난유형 사진신고*/}
            <div className="write" style={styleColor}>
                재난 유형<span className="chooseType">(필수*)</span>
            </div>
            ...
            <div style={styleCam1}>
                <div className="write" style={styleColor}>
                    현장 사진 접수<span className="choose">(필수*)</span>
                </div>
                ...
                <img id="show1" style={styleImgShow1}/>
            </div>
            <br /><br />
        </div>
        ...
        <div className="box" style={styleMap}> {/*위치, 지도, 불편내용 접수, 등록버튼*/}
            ...
            <div className="boxgroup">
                ...
                <div style={styleCam2}>
                    ...
                    <img id="show2" style={styleImgShow2}/>
                    <br /><br /><br />
                </div>
